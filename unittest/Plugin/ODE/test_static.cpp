...
TEST(StaticODESolver, Beam) {
    ...
    setSimulation(new sofa::simulation::graph::DAGSimulation());
    auto root = getSimulation()->createNewNode("root");
    createObject(root, "RequiredPlugin", {{"pluginName", "SofaTopologyMapping SofaBoundaryCondition SofaEngine"}});
    ...
    auto solver = dynamic_cast<SofaCaribou::ode::StaticODESolver *>(
            createObject(meca, "StaticODESolver", {{"newton_iterations", "10"}}).get()
    );
    ...
    std::array<std::vector<double>, 5> force_residuals = {{
            {1.0000000000000000, 0.0000230593394151, 0.0000001195628280, 0.0000000000000041},                     // Step 1
            {1.0000000000000000, 0.0000220904918306, 0.0000001422686414, 0.0000000001663475, 0.0000000000000005}, // Step 2
            {1.0000000000000000, 0.0000195016259904, 0.0000002212510561, 0.0000000013393572, 0.0000000000000061}, // Step 3
            {1.0000000000000000, 0.0000160276776802, 0.0000003922581677, 0.0000000024969770, 0.0000000000000202}, // Step 4
            {1.0000000000000000, 0.0000124393213081, 0.0000006901975979, 0.0000000021782897, 0.0000000000000271}  // Step 5
    }};
    ...
    for (unsigned int step_id = 0; step_id < force_residuals.size(); ++step_id) {
        ...
        for (unsigned int newton_step_id = 0; newton_step_id < solver->squared_residuals().size(); ++newton_step_id) {
            ...
            EXPECT_NEAR(residual, force_residuals[step_id][newton_step_id], 1e-10);
        }
    }
