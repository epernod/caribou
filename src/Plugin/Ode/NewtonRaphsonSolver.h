...
private:
    ...
    /**
     * Compute the initial right-hand side (RHS) of the equation to be solved at the **beginning of the step**.
     ...
     * of each Newton step. The system matrix correspond to the jacobian of a function with respect to the displacement.
    ...
    /**
     * Propagate the newly solved position increment.
     *
     * This method should update the multi-vector dx. Once this is done, it should update the geometry,
     * i.e. x += dx. Finally, it should propagate this multi-vector to mapped mechanical objects through
     * mechanical mappings. In the case of dynamic solvers, this method is also usually responsible to
     * integrate the new time step.
     *
     * @param mechanical_parameters The set of mechanical parameters defined by the Newton-Raphson. It will contains
     * the identifiers of the x, v, f, dx and df multi-vectors, as well as the current dt.
     * @param matrix_accessor The multi-matrix accessor which contains the current mechanical graph.
     ...
     * @param dx_id The displacement multi-vector identifier.
     *
     *
